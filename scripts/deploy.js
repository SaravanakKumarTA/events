...
async function main() {
  const EventTest = await hre.ethers.getContractFactory("Brank");
  const eventTest = await EventTest.deploy();

  (await eventTest).deployed();
  (await eventTest).on("Deposit", (owner, amount) => {
    console.log(`New deposit: ${owner} ${amount} WEI`);
  });

  (await eventTest).on("Withdraw", (owner, amount) => {
    console.log(`New withdraw: ${owner} ${amount} WEI`);
  });

  (await eventTest).on("Transfer", (from, to, amount) => {
    console.log(`New transfer: ${from} ${to} ${amount} WEI`);
  });
